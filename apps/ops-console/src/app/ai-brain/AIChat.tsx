...
export default function AIChat({ userRole }: { userRole: string }) {
  ...
  return (
    <div className="flex flex-col h-full">
      {messages.length === 0 && (
        <div className="mb-4">
          <p className="text-xs text-[#5b6b85] uppercase tracking-wider font-bold mb-2">Try asking</p>
          <div className="flex flex-wrap gap-2">
            {EXAMPLE_QUESTIONS.map((q) => (
              <button
                ...
                onClick={() => handleAsk(q)}
                disabled={loading}
                className="text-xs px-3 py-1.5 rounded-lg border border-[rgba(176,27,66,0.15)] text-[#5b6b85] hover:border-[#b01b42] hover:text-[#d9647f] transition-colors disabled:opacity-50"
              >
                {q}
              </button>
            ))}
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto space-y-3 mb-3 max-h-[500px]">
        {messages.map((msg, i) => (
          ...
          >
            <p className="text-[10px] text-[#8b97ab] mb-1">
              {msg.role === "user" ? "You" : "AI Brain"} · {msg.timestamp.toLocaleTimeString()}
            </p>
            <div className="text-[#16233c] whitespace-pre-wrap">{msg.content}</div>
          </div>
        ))}
        {loading && (
          <div className="bg-[#f4f6fa] rounded-lg p-3 mr-8">
            <p className="text-sm text-[#5b6b85] animate-pulse">Analyzing your data…</p>
          </div>
        )}
      </div>
      ...
      <div className="flex gap-2">
        <input
          ...
          className="flex-1 bg-[#f4f6fa] border border-[rgba(176,27,66,0.15)] rounded-lg px-3 py-2 text-sm placeholder:text-[#8b97ab]"
          disabled={loading}
        />
        <button
          onClick={() => handleAsk()}
