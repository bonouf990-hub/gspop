...
export default function SubmissionForm({
  ...
  if (!verified) {
    return (
      <div className="border border-[#b01b42] bg-[#ffffff] rounded-xl p-6">
        ...
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 bg-[#f4f6fa] border border-[rgba(176,27,66,0.15)] rounded-lg px-3 py-2.5 text-sm"
  ...
  if (submitted) {
    return (
      <div className="border border-green-700 bg-green-950/30 rounded-xl p-8 text-center">
        <p className="text-2xl font-extrabold text-green-700 mb-2">Submission Received</p>
  ...
  return (
    <div className="border border-[#b01b42] bg-[#ffffff] rounded-xl p-6">
      ...
      <div className="space-y-4">
        <div>
          <p className="text-[10px] text-[#5b6b85] uppercase tracking-wider font-bold mb-2">Company Details</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <input
              placeholder="Company Name *"
              value={vendorName}
              onChange={(e) => setVendorName(e.target.value)}
              className="bg-[#f4f6fa] border border-[rgba(176,27,66,0.15)] rounded-lg px-3 py-2.5 text-sm"
            />
            <input
        ...
        <div>
          <p className="text-[10px] text-[#5b6b85] uppercase tracking-wider font-bold mb-2">Bid Details</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <input
        ...
        <div>
          <p className="text-[10px] text-[#5b6b85] uppercase tracking-wider font-bold mb-2">Cover Letter</p>
          <textarea
            placeholder="Introduce your company and why you're the best fit for this project…"
            value={coverLetter}
            onChange={(e) => setCoverLetter(e.target.value)}
            rows={4}
            className="w-full bg-[#f4f6fa] border border-[rgba(176,27,66,0.15)] rounded-lg px-3 py-2.5 text-sm"
          />
        </div>

        <div>
          <p className="text-[10px] text-[#5b6b85] uppercase tracking-wider font-bold mb-2">Technical Approach</p>
          <textarea
        ...
        {requirements.length > 0 && (
          <div>
            <p className="text-[10px] text-[#5b6b85] uppercase tracking-wider font-bold mb-2">
              Requirement Responses
            </p>
            <div className="space-y-3">
              {requirements.map((req) => (
                <div key={req.id}>
                  <label className="text-sm font-medium text-[#16233c] flex items-center gap-1">
                    {req.title}
                    {req.is_mandatory && <span className="text-red-600">*</span>}
                  </label>
                  {req.description && (
                    <p className="text-xs text-[#8b97ab] mb-1">{req.description}</p>
                  )}
                  <textarea
                    ...
                    rows={2}
                    className="w-full bg-[#f4f6fa] border border-[rgba(176,27,66,0.15)] rounded-lg px-3 py-2 text-sm"
                  />
                </div>
              ))}
            </div>
