  ...
  return (
    <main className="min-h-screen bg-[#0B1320] text-white p-6">
      ...
      <ul className="space-y-3">
        {complaints.map((c) => (
          <li key={c.id} className="bg-[#162335] rounded-xl p-4">
            <p className="font-medium">{c.title}</p>
            <p className="text-sm text-gray-400">{c.status.replace(/_/g, " ")}</p>
          </li>
        ))}
        {complaints.length === 0 && <p className="text-gray-500">No requests submitted yet.</p>}
      </ul>
    </main>
  );
}
